...
<body>
    ...
    <form action="{{route('enviar.Email')}}" method="post" autocomplete="off" class="form"> 
        @csrf
        <fieldset class="form__fieldset">
            <legend class="form__title">Informe seu email para que possamos enviar um link para redefinir sua senha.</legend>
            @include('mensagem')
            <label for="user" class="form__label">E-mail</label>
            <input type="email"  name="email" required class="form__input" />
            <input type="submit" value="Enviar" class="btn btn-primary" />
            <a href="{{route('login.auth')}}">Voltar para login</a>
        </fieldset>
    </form>
</body>
